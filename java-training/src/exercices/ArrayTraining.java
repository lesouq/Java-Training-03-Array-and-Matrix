public class ArrayTraining {
    ...
    public int sum(int[] array) {
            int arraySum = 0;
            for (int arraylength = 0; arraylength < array.length; arraylength++){
                arraySum += array[arraylength];
            }
        return arraySum;
    }
    ...
    public String[] capitalize(String[] array) {
        for(int arrayLength = 0; arrayLength < array.length; arrayLength++){
            String capitalizedWorld = array[arrayLength].substring(0,1).toUpperCase() + array[arrayLength].substring(1);
            array[arrayLength] = capitalizedWorld;
        }
        return array;
    }
    ...
    public boolean equals(int[] first, int[] second) {
        int matchCount = 0;
		boolean same = false;		
		for (int arrayLength = 0 ; arrayLength < first.length ; arrayLength++){
			if (first[arrayLength] == second[arrayLength]){
				matchCount += 1;
			}
		}
			if (matchCount == first.length){
				same = true;
			}
        return same;
    }
    ...
    public int[] reverse(int[] array) {
		...
		int reversedIndx = array.length - 1;
		for (int i = 0 ; i < array.length ; i++){	
			reversedArray[i] = array[reversedIndx];
			reversedIndx--;
		}
    ...
    public int[] concat(int[] first, int[] second) {
        int[] mergedArrays = new int[first.length + second.length];      
        for(int firstIndex = 0; firstIndex < first.length; firstIndex++) { 
           mergedArrays[firstIndex] = first[firstIndex];         
        } 
        for(int secondIndex = 0 ; secondIndex < second.length ; secondIndex++){ 
           mergedArrays[secondIndex + first.length] = second[secondIndex];
        }   
        return mergedArrays;
    }
}
